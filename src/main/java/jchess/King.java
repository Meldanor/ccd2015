...
public class King extends Piece {
    ...
    public ArrayList allMoves() {
        ...
        if (!this.wasMotion && !this.isChecked()) {//check if king was not moved before
            ...
            if (chessboard.squares[0][this.square.pozY].piece != null
                ...
                if (!rook.wasMotion) {
                    ...
                    if (canCastling && this.isSafe(sq) && this.isSafe(sq1)) { //can do castling when none of Sq,sq1 is checked
                        list.add(sq);
                    }
                }
            }
            if (chessboard.squares[7][this.square.pozY].piece != null
                ...
                if (!rook.wasMotion) {//if king was not moves before and is not checked
                    for (int i = this.square.pozX + 1; i < 7; i++) {//go right
                        if (chessboard.squares[i][this.square.pozY].piece != null) {//if square is not empty
